// sjlj2.cpp : このファイルには 'main' 関数が含まれています。プログラム実行の開始と終了がそこで行われます。
//

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <setjmp.h>
#include <coroutine>


//module test1;
//import MyFunc;

enum class value_type {
	null = 0,
	uc8,
	sc8,
	us16,
	ss16,
	ui32,
	si32,
	ul64,
	sl64,
	ptr,
	msg,
	flt,
	dbl,
};

typedef union {
	unsigned char bytes[4];
	unsigned char uc8;
	char sc8;
	unsigned short us16;
	short ss16;
	unsigned int ui32;
	int si32;
	unsigned long ul64;
	long sl64;
	void* ptr;
	char* msg;
	float flt;
	double dbl;
} element;

class vobj {
private:

	vobj* add(unsigned long value) {
		switch (this->type) {
		case value_type::msg:
			break;

		case value_type::uc8:
		case value_type::sc8:
			this->element.uc8 += (unsigned char)value;
			break;

		case value_type::us16:
		case value_type::ss16:
			this->element.us16 += (unsigned short)value;
			break;

		case value_type::ui32:
		case value_type::si32:
			this->element.ui32 += (unsigned int)value;
			break;

		case value_type::ul64:
		case value_type::sl64:
			this->element.ul64 += (unsigned long)value;
			break;

		case value_type::ptr:
			this->element.ptr = (void*)((size_t)(this->element.ptr) + (size_t)value);
			break;

		case value_type::flt:
			this->element.flt += (float)value;
			break;

		case value_type::dbl:
			this->element.dbl += (double)value;
			break;
		}
		hash = 0;
		return this;
	}

	vobj* sub(unsigned long value) {
		switch (this->type) {
		case value_type::msg:
			break;

		case value_type::uc8:
		case value_type::sc8:
			this->element.uc8 -= (unsigned char)value;
			break;

		case value_type::us16:
		case value_type::ss16:
			this->element.us16 -= (unsigned short)value;
			break;

		case value_type::ui32:
		case value_type::si32:
			this->element.ui32 -= (unsigned int)value;
			break;

		case value_type::ul64:
		case value_type::sl64:
			this->element.ul64 -= (unsigned long)value;
			break;

		case value_type::ptr:
			this->element.ptr = (void*)((size_t)(this->element.ptr) - (size_t)value);
			break;

		case value_type::flt:
			this->element.flt -= (float)value;
			break;

		case value_type::dbl:
			this->element.dbl -= (double)value;
			break;
		}
		hash = 0;
		return this;
	}

	vobj* mul(unsigned long value) {
		switch (this->type) {
		case value_type::msg:
			break;

		case value_type::uc8:
		case value_type::sc8:
			this->element.uc8 *= (unsigned char)value;
			break;

		case value_type::us16:
		case value_type::ss16:
			this->element.us16 *= (unsigned short)value;
			break;

		case value_type::ui32:
		case value_type::si32:
			this->element.ui32 *= (unsigned int)value;
			break;

		case value_type::ul64:
		case value_type::sl64:
			this->element.ul64 *= (unsigned long)value;
			break;

		case value_type::ptr:
			this->element.ptr = (void*)((size_t)(this->element.ptr) * (size_t)value);
			break;

		case value_type::flt:
			this->element.flt *= (float)value;
			break;

		case value_type::dbl:
			this->element.dbl *= (double)value;
			break;
		}
		hash = 0;
		return this;
	}

	vobj* div(unsigned long value) {
		switch (this->type) {
		case value_type::msg:
			break;

		case value_type::uc8:
		case value_type::sc8:
			this->element.uc8 /= (unsigned char)value;
			break;

		case value_type::us16:
		case value_type::ss16:
			this->element.us16 /= (unsigned short)value;
			break;

		case value_type::ui32:
		case value_type::si32:
			this->element.ui32 /= (unsigned int)value;
			break;

		case value_type::ul64:
		case value_type::sl64:
			this->element.ul64 /= (unsigned long)value;
			break;

		case value_type::ptr:
			this->element.ptr = (void*)((size_t)(this->element.ptr) / (size_t)value);
			break;

		case value_type::flt:
			this->element.flt /= (float)value;
			break;

		case value_type::dbl:
			this->element.dbl /= (double)value;
			break;
		}
		hash = 0;
		return this;
	}

	vobj* mod(unsigned long value) {
		switch (this->type) {
		case value_type::msg:
			break;

		case value_type::uc8:
		case value_type::sc8:
			this->element.uc8 %= (unsigned char)value;
			break;

		case value_type::us16:
		case value_type::ss16:
			this->element.us16 %= (unsigned short)value;
			break;

		case value_type::ui32:
		case value_type::si32:
			this->element.ui32 %= (unsigned int)value;
			break;

		case value_type::ul64:
		case value_type::sl64:
			this->element.ul64 %= (unsigned long)value;
			break;

		case value_type::ptr:
			this->element.ptr = (void*)((size_t)(this->element.ptr) % (size_t)value);
			break;

		case value_type::flt:
			//this->element.flt %= (float)value;
			break;

		case value_type::dbl:
			//this->element.dbl %= (double)value;
			break;
		}
		hash = 0;
		return this;
	}

	const unsigned int crc32tab[256] = {
	0x00000000, 0x77073096, 0xee0e612c, 0x990951ba,
	0x076dc419, 0x706af48f, 0xe963a535, 0x9e6495a3,
	0x0edb8832, 0x79dcb8a4, 0xe0d5e91e, 0x97d2d988,
	0x09b64c2b, 0x7eb17cbd, 0xe7b82d07, 0x90bf1d91,
	0x1db71064, 0x6ab020f2, 0xf3b97148, 0x84be41de,
	0x1adad47d, 0x6ddde4eb, 0xf4d4b551, 0x83d385c7,
	0x136c9856, 0x646ba8c0, 0xfd62f97a, 0x8a65c9ec,
	0x14015c4f, 0x63066cd9, 0xfa0f3d63, 0x8d080df5,
	0x3b6e20c8, 0x4c69105e, 0xd56041e4, 0xa2677172,
	0x3c03e4d1, 0x4b04d447, 0xd20d85fd, 0xa50ab56b,
	0x35b5a8fa, 0x42b2986c, 0xdbbbc9d6, 0xacbcf940,
	0x32d86ce3, 0x45df5c75, 0xdcd60dcf, 0xabd13d59,
	0x26d930ac, 0x51de003a, 0xc8d75180, 0xbfd06116,
	0x21b4f4b5, 0x56b3c423, 0xcfba9599, 0xb8bda50f,
	0x2802b89e, 0x5f058808, 0xc60cd9b2, 0xb10be924,
	0x2f6f7c87, 0x58684c11, 0xc1611dab, 0xb6662d3d,
	0x76dc4190, 0x01db7106, 0x98d220bc, 0xefd5102a,
	0x71b18589, 0x06b6b51f, 0x9fbfe4a5, 0xe8b8d433,
	0x7807c9a2, 0x0f00f934, 0x9609a88e, 0xe10e9818,
	0x7f6a0dbb, 0x086d3d2d, 0x91646c97, 0xe6635c01,
	0x6b6b51f4, 0x1c6c6162, 0x856530d8, 0xf262004e,
	0x6c0695ed, 0x1b01a57b, 0x8208f4c1, 0xf50fc457,
	0x65b0d9c6, 0x12b7e950, 0x8bbeb8ea, 0xfcb9887c,
	0x62dd1ddf, 0x15da2d49, 0x8cd37cf3, 0xfbd44c65,
	0x4db26158, 0x3ab551ce, 0xa3bc0074, 0xd4bb30e2,
	0x4adfa541, 0x3dd895d7, 0xa4d1c46d, 0xd3d6f4fb,
	0x4369e96a, 0x346ed9fc, 0xad678846, 0xda60b8d0,
	0x44042d73, 0x33031de5, 0xaa0a4c5f, 0xdd0d7cc9,
	0x5005713c, 0x270241aa, 0xbe0b1010, 0xc90c2086,
	0x5768b525, 0x206f85b3, 0xb966d409, 0xce61e49f,
	0x5edef90e, 0x29d9c998, 0xb0d09822, 0xc7d7a8b4,
	0x59b33d17, 0x2eb40d81, 0xb7bd5c3b, 0xc0ba6cad,
	0xedb88320, 0x9abfb3b6, 0x03b6e20c, 0x74b1d29a,
	0xead54739, 0x9dd277af, 0x04db2615, 0x73dc1683,
	0xe3630b12, 0x94643b84, 0x0d6d6a3e, 0x7a6a5aa8,
	0xe40ecf0b, 0x9309ff9d, 0x0a00ae27, 0x7d079eb1,
	0xf00f9344, 0x8708a3d2, 0x1e01f268, 0x6906c2fe,
	0xf762575d, 0x806567cb, 0x196c3671, 0x6e6b06e7,
	0xfed41b76, 0x89d32be0, 0x10da7a5a, 0x67dd4acc,
	0xf9b9df6f, 0x8ebeeff9, 0x17b7be43, 0x60b08ed5,
	0xd6d6a3e8, 0xa1d1937e, 0x38d8c2c4, 0x4fdff252,
	0xd1bb67f1, 0xa6bc5767, 0x3fb506dd, 0x48b2364b,
	0xd80d2bda, 0xaf0a1b4c, 0x36034af6, 0x41047a60,
	0xdf60efc3, 0xa867df55, 0x316e8eef, 0x4669be79,
	0xcb61b38c, 0xbc66831a, 0x256fd2a0, 0x5268e236,
	0xcc0c7795, 0xbb0b4703, 0x220216b9, 0x5505262f,
	0xc5ba3bbe, 0xb2bd0b28, 0x2bb45a92, 0x5cb36a04,
	0xc2d7ffa7, 0xb5d0cf31, 0x2cd99e8b, 0x5bdeae1d,
	0x9b64c2b0, 0xec63f226, 0x756aa39c, 0x026d930a,
	0x9c0906a9, 0xeb0e363f, 0x72076785, 0x05005713,
	0x95bf4a82, 0xe2b87a14, 0x7bb12bae, 0x0cb61b38,
	0x92d28e9b, 0xe5d5be0d, 0x7cdcefb7, 0x0bdbdf21,
	0x86d3d2d4, 0xf1d4e242, 0x68ddb3f8, 0x1fda836e,
	0x81be16cd, 0xf6b9265b, 0x6fb077e1, 0x18b74777,
	0x88085ae6, 0xff0f6a70, 0x66063bca, 0x11010b5c,
	0x8f659eff, 0xf862ae69, 0x616bffd3, 0x166ccf45,
	0xa00ae278, 0xd70dd2ee, 0x4e048354, 0x3903b3c2,
	0xa7672661, 0xd06016f7, 0x4969474d, 0x3e6e77db,
	0xaed16a4a, 0xd9d65adc, 0x40df0b66, 0x37d83bf0,
	0xa9bcae53, 0xdebb9ec5, 0x47b2cf7f, 0x30b5ffe9,
	0xbdbdf21c, 0xcabac28a, 0x53b39330, 0x24b4a3a6,
	0xbad03605, 0xcdd70693, 0x54de5729, 0x23d967bf,
	0xb3667a2e, 0xc4614ab8, 0x5d681b02, 0x2a6f2b94,
	0xb40bbe37, 0xc30c8ea1, 0x5a05df1b, 0x2d02ef8d,
	};
	unsigned int hash;

	unsigned int crc32(char* p, int len, unsigned int result)
	{
		result ^= 0xFFFFFFFF;
		for (; len--; p++) {
			result = ((result >> 8) & 0x00FFFFFF) ^ crc32tab[(result ^ (*p)) & 0xFF];
		}
		result ^= 0xFFFFFFFF;
		return result;
	}

public:
	value_type type;
	element element;
	int length;

	unsigned int get_hash() {
		if (0 == hash) {
			hash = (type == value_type::msg) ? crc32(element.msg, strlen(element.msg), (int)type) : crc32((char*)element.bytes, 4, (int)type);
		}
		return hash;
	}

	void set_value(unsigned char uc8) {
		this->type = value_type::uc8;
		this->element.uc8 = uc8;
		hash = 0;
	}

	void set_value(char sc8) {
		this->type = value_type::sc8;
		this->element.sc8 = sc8;
		hash = 0;
	}

	void set_value(unsigned short us16) {
		this->type = value_type::us16;
		this->element.us16 = us16;
		hash = 0;
	}

	void set_value(short ss16) {
		this->type = value_type::ss16;
		this->element.ss16 = ss16;
		hash = 0;
	}

	void set_value(unsigned int ui32) {
		this->type = value_type::ui32;
		this->element.ui32 = ui32;
		hash = 0;
	}

	void set_value(int si32) {
		this->type = value_type::si32;
		this->element.si32 = si32;
		hash = 0;
	}

	void set_value(unsigned long ul64) {
		this->type = value_type::ul64;
		this->element.ul64 = ul64;
		hash = 0;
	}

	void set_value(long sl64) {
		this->type = value_type::sl64;
		this->element.sl64 = sl64;
		hash = 0;
	}

	void set_value(void* ptr) {
		this->type = value_type::ptr;
		this->element.ptr = ptr;
		hash = 0;
	}

	void set_value(char* msg) {
		this->type = value_type::msg;
		this->element.msg = msg;
		hash = 0;
	}

	void set_value(float flt) {
		this->type = value_type::flt;
		this->element.flt = flt;
		hash = 0;
	}

	void set_value(double dbl) {
		this->type = value_type::dbl;
		this->element.dbl = dbl;
		hash = 0;
	}

	vobj* operator +(unsigned char value) { return add(value); }
	vobj* operator +(char value) { return add(value); }
	vobj* operator +(unsigned short value) { return add(value); }
	vobj* operator +(short value) { return add(value); }
	vobj* operator +(unsigned int value) { return add(value); }
	vobj* operator +(int value) { return add(value); }
	vobj* operator +(unsigned long value) { return add(value); }
	vobj* operator +(long value) { return add(value); }
	vobj* operator +=(unsigned char value) { return add(value); }
	vobj* operator +=(char value) { return add(value); }
	vobj* operator +=(unsigned short value) { return add(value); }
	vobj* operator +=(short value) { return add(value); }
	vobj* operator +=(unsigned int value) { return add(value); }
	vobj* operator +=(int value) { return add(value); }
	vobj* operator +=(unsigned long value) { return add(value); }
	vobj* operator +=(long value) { return add(value); }

	vobj* operator -(unsigned char value) { return add(value); }
	vobj* operator -(char value) { return add(value); }
	vobj* operator -(unsigned short value) { return add(value); }
	vobj* operator -(short value) { return add(value); }
	vobj* operator -(unsigned int value) { return add(value); }
	vobj* operator -(int value) { return add(value); }
	vobj* operator -(unsigned long value) { return add(value); }
	vobj* operator -(long value) { return add(value); }
	vobj* operator -=(unsigned char value) { return add(value); }
	vobj* operator -=(char value) { return add(value); }
	vobj* operator -=(unsigned short value) { return add(value); }
	vobj* operator -=(short value) { return add(value); }
	vobj* operator -=(unsigned int value) { return add(value); }
	vobj* operator -=(int value) { return add(value); }
	vobj* operator -=(unsigned long value) { return add(value); }
	vobj* operator -=(long value) { return add(value); }

	vobj* operator *(unsigned char value) { return add(value); }
	vobj* operator *(char value) { return add(value); }
	vobj* operator *(unsigned short value) { return add(value); }
	vobj* operator *(short value) { return add(value); }
	vobj* operator *(unsigned int value) { return add(value); }
	vobj* operator *(int value) { return add(value); }
	vobj* operator *(unsigned long value) { return add(value); }
	vobj* operator *(long value) { return add(value); }
	vobj* operator *=(unsigned char value) { return add(value); }
	vobj* operator *=(char value) { return add(value); }
	vobj* operator *=(unsigned short value) { return add(value); }
	vobj* operator *=(short value) { return add(value); }
	vobj* operator *=(unsigned int value) { return add(value); }
	vobj* operator *=(int value) { return add(value); }
	vobj* operator *=(unsigned long value) { return add(value); }
	vobj* operator *=(long value) { return add(value); }

	vobj* operator /(unsigned char value) { return add(value); }
	vobj* operator /(char value) { return add(value); }
	vobj* operator /(unsigned short value) { return add(value); }
	vobj* operator /(short value) { return add(value); }
	vobj* operator /(unsigned int value) { return add(value); }
	vobj* operator /(int value) { return add(value); }
	vobj* operator /(unsigned long value) { return add(value); }
	vobj* operator /(long value) { return add(value); }
	vobj* operator /=(unsigned char value) { return add(value); }
	vobj* operator /=(char value) { return add(value); }
	vobj* operator /=(unsigned short value) { return add(value); }
	vobj* operator /=(short value) { return add(value); }
	vobj* operator /=(unsigned int value) { return add(value); }
	vobj* operator /=(int value) { return add(value); }
	vobj* operator /=(unsigned long value) { return add(value); }
	vobj* operator /=(long value) { return add(value); }

	vobj* operator %(unsigned char value) { return add(value); }
	vobj* operator %(char value) { return add(value); }
	vobj* operator %(unsigned short value) { return add(value); }
	vobj* operator %(short value) { return add(value); }
	vobj* operator %(unsigned int value) { return add(value); }
	vobj* operator %(int value) { return add(value); }
	vobj* operator %(unsigned long value) { return add(value); }
	vobj* operator %(long value) { return add(value); }
	vobj* operator %=(unsigned char value) { return add(value); }
	vobj* operator %=(char value) { return add(value); }
	vobj* operator %=(unsigned short value) { return add(value); }
	vobj* operator %=(short value) { return add(value); }
	vobj* operator %=(unsigned int value) { return add(value); }
	vobj* operator %=(int value) { return add(value); }
	vobj* operator %=(unsigned long value) { return add(value); }
	vobj* operator %=(long value) { return add(value); }
};

typedef struct {
	vobj* key;
	vobj* val;
} vpair;

class dic {
private:
	vpair* pairs;
	int last;
	int length;

public:
	dic(int length) {
		this->pairs = (vpair*)malloc(sizeof(dic) * length);
		this->last = 0;
		this->length = length;
	}

	vpair* get_pair(vobj* key) {
		auto end = &pairs[length];		
		auto hash = key->get_hash();
		for (vpair* cur = pairs; cur < end; cur++) {
			if (cur->key->get_hash() == hash)
				return cur;
		}
		return NULL;
	}

	vobj* get_value(vobj* key) {
		vpair* pair = get_pair(key);
		return (NULL == pair) ? NULL : pair->val;
	}

	void set_pair(vobj* key, vobj* val) {
		vpair* pair = get_pair(key);
		if (NULL == pair) {
			vpair* cur = &this->pairs[last++];
			cur->key = key;
			cur->val = val;
		}
	}

};

class queue {
private:
	vobj** arr;
	int length;
	int first;
	int last;

public:
	queue(int length) {
		this->arr = new vobj*[length];
		this->length = length;
		this->first = 0;
		this->last = 0;
	}

	~queue() {
		if (NULL != arr) {
			delete this->arr;
			arr = NULL;
		}
	}

	bool try_enque(vobj* value) {
		int next = (this->last + 1) % this->length;
		if (this->first == next)
			return false;

		this->arr[this->last] = value;
		this->last = next;
		return true;
	}

	bool try_dequeue(vobj** result) {
		if (this->first == this->last)
			return false;

		*result = this->arr[this->first];
		this->first = (this->first + 1) % this->length;
		return true;
	}
};




int main()
{
	queue* que = new queue(10);

	vobj v1;
	vobj v2;
	v1.set_value(1234);
	v2.set_value(4321);

	que->try_enque(&v1);
	que->try_enque(&v2);

	vobj* a1;
	vobj* a2;

	que->try_dequeue(&a1);
	que->try_dequeue(&a2);

	v1 += 50000;

	printf("a1=%d\n", a1->element.ui32);
	printf("a2=%d\n", a2->element.ui32);
}






// プログラムの実行: Ctrl + F5 または [デバッグ] > [デバッグなしで開始] メニュー
// プログラムのデバッグ: F5 または [デバッグ] > [デバッグの開始] メニュー

// 作業を開始するためのヒント: 
//    1. ソリューション エクスプローラー ウィンドウを使用してファイルを追加/管理します 
//   2. チーム エクスプローラー ウィンドウを使用してソース管理に接続します
//   3. 出力ウィンドウを使用して、ビルド出力とその他のメッセージを表示します
//   4. エラー一覧ウィンドウを使用してエラーを表示します
//   5. [プロジェクト] > [新しい項目の追加] と移動して新しいコード ファイルを作成するか、[プロジェクト] > [既存の項目の追加] と移動して既存のコード ファイルをプロジェクトに追加します
//   6. 後ほどこのプロジェクトを再び開く場合、[ファイル] > [開く] > [プロジェクト] と移動して .sln ファイルを選択します
